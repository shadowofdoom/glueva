...
run_bridge_hook() {
  ...
  bin="$(resolve_bridge_bin)" || return 0

  if output="$("${bin}" hook "${event}" --plugin-protocol "${BRIDGE_PROTOCOL}")"; then
    [ -n "${output}" ] && printf '%s\n' "${output}"
    return 0
  fi
  ...
  if [ "${event}" = "session-start" ]; then
    printf '%s\n' '{"hookSpecificOutput":{"hookEventName":"SessionStart","additionalContext":"glueva: the CLI is installed but its hook integration failed. This session is NOT bridged; reinstall the CLI and plugin so their protocol versions match."}}'
  else
