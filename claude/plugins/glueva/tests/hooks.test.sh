...
# covered in bridge/test/claude-hooks.test.ts; these checks prove the plugin
...
rm -f "${STUB}/glueva"
empty "SessionStart is silent" "$(run bridge-session-start.sh '{"session_id":"s1"}')"
empty "Stop is silent" "$(run bridge-stop.sh '{"session_id":"s1"}')"

echo "== CLI present: exact protocol call and stdin forwarding =="
install_stub
out="$(run bridge-session-start.sh '{"session_id":"s1"}' STUB_OUTPUT='{"ok":"start"}')"
check "SessionStart returns CLI output" "${out}" '"start"'
check "SessionStart argv" "$(cat "${TMP}/calls")" "hook session-start --plugin-protocol 1"
check "SessionStart stdin" "$(cat "${TMP}/stdin")" '"session_id":"s1"'
out="$(run bridge-stop.sh '{"session_id":"s2"}' STUB_OUTPUT='{"decision":"block"}')"
check "Stop returns CLI output" "${out}" '"block"'
check "Stop argv" "$(cat "${TMP}/calls")" "hook stop --plugin-protocol 1"
check "Stop stdin" "$(cat "${TMP}/stdin")" '"session_id":"s2"'

echo "== CLI broken: loud, never blocking Stop =="
out="$(run bridge-session-start.sh '{}' STUB_EXIT=7)"
check "SessionStart warns" "${out}" "NOT bridged"
out="$(run bridge-stop.sh '{}' STUB_EXIT=7)"
